...
def get_companies(apikey, properties_type, list_input):
    if properties_type == 'Standard':
        parameter_dict = {'hapikey': apikey, 'limit': const.COMPANIES_LIMIT}
    elif properties_type == 'All':
        company_properties = get_company_properties(apikey)
        parameter_dict = {'hapikey': apikey, 'limit': const.COMPANIES_LIMIT, 'properties': company_properties}
    elif properties_type == 'Custom':
        company_properties = list_input
        parameter_dict = {'hapikey': apikey, 'limit': const.COMPANIES_LIMIT, 'properties': list_input}
    has_more = True
    counter = 0
    while has_more:
        try:
